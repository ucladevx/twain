...
async function getTodaysFreeIntervals() {
    var set_flag = false;
    let free_intervals = [];
    ...
    return new Promise((res, rej) => {
        chrome.identity.getAuthToken({ interactive: true }, async function (token) {
            ...
            var url =
                "https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMax=" +
                time_midnight_string +
                "&timeMin=" +
                time_now_string +
            ...
            var tasks = json["items"];
            free_intervals = getFreeIntervals(tasks)
            res(free_intervals)
            // return json["items"];
...
function getHourDifference(start, end) {
    return (
        ...
        start.getUTCHours() +
        (end.getUTCMinutes() - start.getUTCMinutes()) / 60
    );
}
...
function getFreeIntervals(events) {
    ...
    for (var i = 0; i < events.length; i++) {
        ...
        if (event_start > latestFreeInterval.start) {
            ...
            latestFreeInterval.duration = getHourDifference(
                latestFreeInterval.start,
                latestFreeInterval.end
            );
...
export default getTodaysFreeIntervals;
